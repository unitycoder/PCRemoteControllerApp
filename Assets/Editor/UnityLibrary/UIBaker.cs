...
namespace UnityLibrary.Generators
{
    public class UIBaker : EditorWindow
    {
        ...
        private void CreateShadowImageObject(Image targetImage, Sprite shadowSprite)
        {
            ...
            // Shadow sprite's pivot is centered on the original sprite's visual center
            // We need to adjust position to account for the size difference
            Vector2 targetPivot = targetRect.pivot;
            Vector2 shadowPivot = shadowSprite.pivot;

            // Pixel offset from shadow sprite's pivot to target sprite's visual center
            float pixelsPerUnit = targetImage.sprite != null ? targetImage.sprite.pixelsPerUnit : 100f;

            // The visual offset needed in anchored position units
            Vector2 pivotOffsetPixels = new Vector2(
                (shadowPivot.x - targetPivot.x) * shadowWidth,
                (shadowPivot.y - targetPivot.y) * shadowHeight
            );

            Vector2 pivotOffsetUnits = pivotOffsetPixels / pixelsPerUnit;

            // Apply position with pivot correction
            shadowRect.anchoredPosition = targetRect.anchoredPosition - pivotOffsetUnits;
