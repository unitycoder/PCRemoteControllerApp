Shader "Hidden/PostProcessing/ScreenSpaceHighlightsEdgeOnlyOffset"
{
    HLSLINCLUDE
        ...
        float2 _StreakOffset;
        float _EdgeThickness;
        float _EdgeSharpness;
        float4 _StreakColor;
        
        float3 _IncludeColor;
        float _ColorTolerance;
        int _UseColorInclusion;
        ...
        {
            float3 diff = col1 - col2;
            return sqrt(dot(diff, diff));
        }
        
        float GetColorInclusionMask(float3 color)
        {
            if (_UseColorInclusion == 0)
                return 1.0;
            
            float dist = ColorDistance(color, _IncludeColor);
            
            // Smooth transition based on tolerance
            // If distance < tolerance, returns 1 (included)
            // If distance > tolerance, returns 0 (excluded)
            float mask = 1.0 - smoothstep(0.0, _ColorTolerance, dist);
        ...
        float DetectLuminosityEdge(float2 uv)
        {
            ...
            // Check if center pixel should be included by color
            float colorMask = GetColorInclusionMask(centerSample);
        ...
        float4 Frag(VaryingsDefault i) : SV_Target
        {
            ...
            // Sample streak texture with XY offset
            float2 streakUV = i.texcoord + _StreakOffset;
            streakUV = frac(streakUV);
